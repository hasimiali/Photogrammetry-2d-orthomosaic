...
def compute_homography_metrics(img1, img2, ratio_thresh=0.75):
    ...
    index_params = dict(
        ...
        multi_probe_level=2   # number of bits to shift to check for nearby buckets
    )
    search_params = dict(checks=50)

    flann = cv2.FlannBasedMatcher(index_params, search_params)

    # FLANN butuh descriptor bertipe uint8 untuk ORB
...
def evaluate_folder_homography(
    image_folder,
    output_csv="homography/rata2/avg_metrics_orb_flann.csv"
):
    image_files = sorted([
