...
# ================= STITCH TWO IMAGES (ORB) =================

def stitch_two_images_orb(img1, img2, debug_matches=False, log_path="orb_two_image_log.csv"):
    ...
    # BFMatcher dengan Hamming (khusus deskriptor biner)
    bf = cv2.BFMatcher(cv2.NORM_HAMMING)
    matches = bf.knnMatch(des1, des2, k=2)

    # Lowe Ratio Test
    good = [m for m, n in matches if m.distance < 0.75 * n.distance]
    print(f"Good matches (ORB): {len(good)}")
    ...
    if debug_matches:
        ...
        cv2.imshow("ORB Feature Matches", match_img)
        cv2.waitKey(0)
        cv2.destroyWindow("ORB Feature Matches")

    src_pts = np.float32([kp1[m.queryIdx].pt for m in good]).reshape(-1, 1, 2)
    dst_pts = np.float32([kp2[m.trainIdx].pt for m in good]).reshape(-1, 1, 2)

    # Homography dengan RANSAC
...
stitched = stitch_two_images_orb(img1, img2, debug_matches=False)

if stitched is not None:
    os.makedirs("homography/orb", exist_ok=True)
    cv2.imwrite("homography/orb/stitched_two_images_orb.jpg", stitched)
    cv2.imshow("ORB Stitched Result", stitched)
    cv2.waitKey(0)
