...
def blend_images(base_img, overlay_img, x, y):
    ...
    base_img[y:y+h, x:x+w] = cv2.add(base_bg, overlay_fg)
    return base_img


# ================= STITCH TWO IMAGES (AKAZE + BFMatcher) =================

def stitch_two_images_akaze_bfmatcher(
    img1,
    img2,
    output_dir="homography/akaze_bf",
    log_path="homography/akaze_bf_log.csv"
):
    ...
    cv2.imwrite(os.path.join(output_dir, "inlier_matches.jpg"), img_inliers)
    ...
    cv2.imwrite(os.path.join(output_dir, "stitched_result.jpg"), result)
...
if __name__ == "__main__":
    ...
    result = stitch_two_images_akaze_bfmatcher(img1, img2)

    if result is not None:
        cv2.imshow("AKAZE + BFMatcher Stitching Result", result)
        cv2.waitKey(0)
