...
def run_photogrammetry(folder):
    ...
    with open("evaluation_sift.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Pair",
            "KP Img1",
        ...
        for i in range(1, len(images)):
            H, stats = evaluate_pair(images[i-1], images[i])
...
result = run_photogrammetry("dataset2")
cv2.imwrite("stitched_sift_final.jpg", result)
