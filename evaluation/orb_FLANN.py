...
def stitch_pair(img1, img2, pair_idx, csv_writer, debug_matches=False):
    ...
    # FLANN untuk ORB (binary descriptor) → LSH
    FLANN_INDEX_LSH = 6
    index_params = dict(
    ...
    if debug_matches:
        ...
        cv2.imshow("ORB+FLANN Matches", match_img)
        cv2.waitKey(0)
...
def stitch_images_from_folder(
    folder_path,
    debug_matches=False,
    log_path="orb_FLANN_pair_evaluation.csv"
):
    image_files = sorted([
    ...
    with open(log_path, "w", newline="") as file:
        ...
        for i in range(1, len(images)):
            print(f"Stitching ORB+FLANN pair {i}")
...
if __name__ == "__main__":
    ...
    result = stitch_images_from_folder(
        folder_path,
        debug_matches=False,
        log_path="evaluation/orb/orb_FLANN_evaluation.csv"
    )

    if result is not None:
        cv2.imwrite("evaluation/orb/stitched_orb_FLANN_output.jpg", result)
        cv2.imshow("ORB+FLANN Stitched Result", result)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
